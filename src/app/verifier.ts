...
const verificationMap: VerificationMap = {
  [Direction.Top]: function verifyTop(
    puzzleMap: string[],
    lineIndex: number,
    symbolIndex: number,
  ) {
    return (
      lineIndex !== 0 &&
      symbolsMap[puzzleMap[lineIndex - 1].split("")[symbolIndex]].includes(
        Direction.Bottom,
      )
    );
  },
  [Direction.Bottom]: function verifyBottom(
    puzzleMap: string[],
    lineIndex: number,
    symbolIndex: number,
  ) {
    return (
      lineIndex < puzzleMap.length - 1 &&
      symbolsMap[puzzleMap[lineIndex + 1].split("")[symbolIndex]].includes(
        Direction.Top,
      )
    );
  },
  [Direction.Left]: function verifyLeft(
    puzzleMap: string[],
    lineIndex: number,
    symbolIndex: number,
  ) {
    return (
      symbolIndex !== 0 &&
      symbolsMap[puzzleMap[lineIndex].split("")[symbolIndex - 1]].includes(
        Direction.Right,
      )
    );
  },
  [Direction.Right]: function verifyRight(
    puzzleMap: string[],
    lineIndex: number,
    symbolIndex: number,
  ) {
    ...
    return (
      symbolIndex < currentLineArray.length - 1 &&
...
export function checkConnections(
  puzzleMap: string[],
  oldConnections: RootState["connections"]["value"],
) {
  let connections = [
    ...oldConnections.map((group) => ({
  ...
  for (let lineIndex = 0; lineIndex < puzzleMap.length; lineIndex++) {
    ...
    for (
      let symbolIndex = 0;
      symbolIndex < currentLineArray.length;
      symbolIndex++
    ) {
      ...
      for (const direction of symbolsMap[currentLineArray[symbolIndex]]) {
        ...
        if (verificationMap[direction](puzzleMap, lineIndex, symbolIndex)) {
          ...
          const groupIndex = connections.findIndex((group) =>
            group.elements.includes(indicesString),
          );
          const otherGroupIndex = connections.findIndex((group) =>
            group.elements.includes(adjacentIndexes),
          );
          if (groupIndex > -1) {
            if (!connections[groupIndex].elements.includes(adjacentIndexes)) {
              if (otherGroupIndex > -1) {
                ...
                connections[groupToUpdate].elements = [
                  ...connections[groupIndex].elements,
                  ...connections[otherGroupIndex].elements,
                ];
            ...
            if (otherGroupIndex > -1) {
              ...
              const newGroup = {
                elements: [indicesString, adjacentIndexes],
                color: `#${generateColor()}`,
              };
...
export function removeOldConnections(
  map: string[],
  connections: RootState["connections"]["value"],
  lineIndex: number,
  segmentIndex: number,
) {
  const currentIndices = `${lineIndex},${segmentIndex}`;
  let groupIndex = connections.findIndex((group) =>
    group.elements.includes(currentIndices),
  );
  ...
  const newDirections = connectionDirections.filter(
    (direction) => !oldDirections.includes(direction),
  );
  const remainingDirections = oldDirections.filter((direction) =>
    connectionDirections.includes(direction),
  );
  ...
  function getNewGroupElements() {
    ...
    for (let i = 0; i < remainingDirections.length; i++) {
      ...
      if (connections[groupIndex].elements.includes(adjacentIndexes)) {
        ...
        const adjacentElement =
          map[parseInt(adjacentIndicesArray[0])].split("")[
            parseInt(adjacentIndicesArray[1])
          ];
        if (adjacentElement) {
          const otherDirections = symbolsMap[adjacentElement];
          for (let j = 0; j < otherDirections.length; j++) {
            const secondAdjacentIndices = getAdjacentIndexes(
              otherDirections[j],
              parseInt(adjacentIndicesArray[0]),
              parseInt(adjacentIndicesArray[1]),
            );
    ...
    for (let i = 0; i < oldDirections.length; i++) {
      ...
      const newGroupIndex = connections.findIndex((group) =>
        group.elements.includes(adjacentIndexes),
      );
      if (newGroupIndex > -1) {
        const adjacentIndicesArray = adjacentIndexes.split(",");
        ...
        for (let j = 0; j < otherDirections.length; j++) {
          ...
          if (secondAdjacentIndices !== currentIndices) {
            ...
            if (
              parseInt(secondAdjacent[0]) < map.length &&
              parseInt(secondAdjacent[1]) < map[0].length &&
              parseInt(secondAdjacent[0]) >= 0 &&
              parseInt(secondAdjacent[1]) >= 0
            ) {
              const secondAdjacentDirections = getOtherDirections(
                map,
                secondAdjacent,
              );
              for (
                let directionIndex = 0;
                directionIndex < secondAdjacentDirections.length;
                directionIndex++
              ) {
                if (
                  getAdjacentIndexes(
                    secondAdjacentDirections[directionIndex],
                    parseInt(secondAdjacent[0]),
                    parseInt(secondAdjacent[1]),
                  ) === adjacentIndexes
                ) {
    ...
    for (let i = 0; i < newDirections.length; i++) {
      ...
      if (
        parseInt(adjacentIndicesArray[0]) < map.length &&
        parseInt(adjacentIndicesArray[1]) < map.length &&
        parseInt(adjacentIndicesArray[0]) >= 0 &&
        parseInt(adjacentIndicesArray[1]) >= 0
      ) {
        const adjacentElement =
          map[parseInt(adjacentIndicesArray[0])].split("")[
            parseInt(adjacentIndicesArray[1])
  ...
  function getOtherDirections(map: string[], adjacentIndexes: string[]) {
    const adjacentElement =
      map[parseInt(adjacentIndexes[0])].split("")[parseInt(adjacentIndexes[1])];
