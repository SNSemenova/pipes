...
test("removes disconnected element", () => {
  const result = removeOldConnections(
    ["┏━┛"],
    [{ elements: ["0,0", "0,1", "0,2"], color: "#000000" }],
    0,
    2,
  );
  expect(result).toHaveLength(1);
...
test("removes disconnected head", () => {
  const result = removeOldConnections(
    ["╺━━┛"],
    [{ elements: ["0,0", "0,1", "0,2", "0,3"], color: "#000000" }],
    0,
    1,
  );
  expect(result).toHaveLength(1);
